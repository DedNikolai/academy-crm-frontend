...
const EditStudent: FC<IStudentItem> = ({student, allTeachers}) => {
    ...
    // const mutation = useStudent(params.id);
    // const {mutate, isPending} = mutation;
    const isPending = false;
    const {register, watch, handleSubmit, reset, formState: {errors}, control} = useForm<IStudent>({
        mode: 'onSubmit', 
    ...
    const onSubmit: SubmitHandler<IStudent> = (data) => {
        const updatedStudent: IStudent = {...data}
                                ...
                                >
                                    {allTeachers.map((teacher) => (
                                        ...
                                        <MenuItem key={teacher._id} value={teacher}>
                                            <Checkbox checked={
                                                !!value.length && value.map(item => item._id).includes(teacher._id)}
                                                 />
